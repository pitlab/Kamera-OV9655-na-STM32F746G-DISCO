...
//Define Server IP
#define ETH_SERVER_IP		"192.168.1.102"
...
void ReadLWIPClientTask(void const *argument)
{
	socket_t gsocket;
	uint16_t n;
	extern uint8_t chBuforTerminala[];
	extern uint8_t chRodzajWyswietlania;
	memset(&gsocket, 0, sizeof(gsocket));
	while(1)
	{
		...
		while(is_sock_connect){
			...
			if(read_len != -1){
				...
				for (n=0; n<read_len; n++)
					chBuforTerminala[n] = *(gsocket.sock_rx_msg.msg + n);
				chRodzajWyswietlania = TW_TERMINAL;
			}
